    ...
    return (
        <div>
            ...
            <div className='flex flex-col items-center lg:flex-row justify-between my-10 '>
                <h1 className='text-2xl pb-5 lg:pb-0 font-bold'> ({sortedItem.length}) Apps Found</h1>
                ...
                {loading && <><span className="loading lg:ml-[550px] loading-bars loading-xl"></span></>}
                {
                    sortedItem.map(p => (
                            <div className='flex flex-col lg:flex-row  justify-between items-center bg-white rounded-2xl shadow-2xl p-2 my-5'>
                                ...
                                <div>
                                    <button onClick={() => handleRemove(p.id)} className='btn btn-active btn-success hover:bg-red-500 hover:scale-110 transition-all duration-300 text-white'>Uninstall</button>
